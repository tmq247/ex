...
async def gban_(SpamX: Client, message: Message):
    ...
    if DATABASE_URL:
       ...
       if check:
         await message.reply_text(f"{user.mention} đã có trong danh sách GBAN!")
         return
...
async def ungban_(SpamX: Client, message: Message):
    ...
    if DATABASE_URL:
       check = gban_db.check(user.id)
       if not check:
         await message.reply_text(f"{user.mention} không có trong danh sách GBAN!")
         return
...
async def glist(SpamX: Client, message: Message):
    ...
    if DATABASE_URL:
       ...
       if len(data) > 0:
          ...
          return
    await message.reply_text("Ah. Bạn đã không điền `DATABASE_URL`")
