...
async def delayspam(SpamX: Client, e: Message): 
    ...
    if LOGS_CHANNEL:
         try:
             await SpamX.send_message(LOGS_CHANNEL, f"đã bắt đầu Trì hoãn thư rác theo người dùng: {e.from_user.id} \n\n Trò chuyện: {e.chat.id} \n Counts: {counts} \n Tin nhắn rác: {msg} \n Thời gian trì hoãn: {sleeptime}")
...
async def pornspam(SpamX: Client, e: Message):
    ...
    if args:
       ...
       if LOGS_CHANNEL:
           try:
               await SpamX.send_message(LOGS_CHANNEL, f"đã bắt đầu Thư rác khiêu dâm của người dùng: {e.from_user.id} \n Trò chuyện: {e.chat.id} \n Counts: {count}")
           except Exception as a:
               print(a)
               pass
    else:
...
async def justspam(SpamX: Client, e: Message):
    ...
    if LOGS_CHANNEL:
         try:
            await SpamX.send_message(LOGS_CHANNEL, f"đã bắt đầu Thư rác của Người dùng: {e.from_user.id} \n\n Trò chuyện: {e.chat.id} \n Counts: {counts} \n Tin nhắn rác: {msg}")
         except Exception as a:
             print(a)
             pass
...
async def hang(SpamX: Client, e: Message): 
    ...
    if LOGS_CHANNEL:
         try:
            await SpamX.send_message(LOGS_CHANNEL, f"đã bắt đầu Treo thư rác theo người dùng: {e.from_user.id} \n\n Chat: {e.chat.id} \n Counts: {count}")
         except Exception as a:
             print(a)
             pass
